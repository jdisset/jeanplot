...
from jeanplot.genetic_elements import (
    ERN, Promoter, TranscriptionUnit, Terminator, UorfGroup,
    Source, ERN5pRecog, GeneticPart, FluoMarker,
)
...
from jeanplot.network_adapter import get_tu_informations_v2, get_interactions_v2, TUInfo
...
class SourceAnnotation(Container):
    ...
    def model_post_init(self, __context: Any = None):
        ...
        if self.has_tag:
            self._source_proxy = Source(
                ...
                source_type=self.source_type, marker=self.marker,
                tag_label=self.tag_label, children=[], parent=self,
            )
    ...
    def render(self, renderer, context, matrix):
        ...
        has_border = self.style.border_width > 0 and self.style.border_color and self.style.border_color.lower() != "none"
...
EL_MAP = {
    "ERN": (ERN, True), "ERN_recog_site_5p": (ERN5pRecog, True),
    "promoter": (Promoter, False), "terminator": (Terminator, False),
    "fluo_marker": (FluoMarker, True), "uORF_group": (UorfGroup, True),
}
CURVE_MAP = {"orthogonal": OrthogonalCurve, "bezier": SimpleBezierCurve, "straight": StraightCurve}


class SchematicComponentFactoryV2:
    def __init__(self, tu_infos: Dict[str, TUInfo], interactions: List[Interaction], connection_style: str):
    ...
    def _create_element(self, part_info: dict, tu_id: str) -> Optional[Component]:
    ...
    def create_connection(self, interaction: Interaction, index: int, parent: Component) -> Optional[Connection]:
        ...
        if not src or not tgt:
            return None
        return Connection(
            id=f"connection_{index}", parent=parent,
            style_class=[f"connection_{interaction.type}", f"connection_{interaction.type}_{interaction.src_part_name}"],
            start_component=src, end_component=tgt,
            curve_type=CURVE_MAP.get(self._connection_style, StraightCurve)(),
            end_cap=LineEndFlat(), is_overlay=True, z_index=10, auto_route=True,
        )
...
class NetworkGeneticSchematicV2(Container):
    ...
    _component_factory: Optional[SchematicComponentFactoryV2] = PrivateAttr(default=None)
    ...
    def _process_network_data(self):
        ...
        tu_to_layer = {}
        for layer_idx, layer_nodes in enumerate(graph.topological_order([n.node_id for n in graph.get_nodes_by_type("translation")])):
        ...
        visible = {t for t in self._tu_infos if not (self.hide_marker_tus and self._tu_infos[t].is_marker)}
        if not visible:
        ...
        for tu_id in visible:
            cotx_by_col[_get_source_id(tu_id, self._tu_infos)][tu_to_layer.get(tu_id, 0)].append(tu_id)
        ...
        sorted_cotx = sorted(cotx_by_col, key=lambda s: (-sum(len(v) for v in cotx_by_col[s].values()),
        ...
        for sid in sorted_cotx:
            ...
            start = 0
            while any((r, c) in occupancy for r in range(start, start + rows_needed) for c in range(min_c, max_c + 1)):
                start += 1
    ...
    def _trace_to_source(self, node_id: int, layer_idx: int, tu_to_layer: dict, visited: set = None):
        if visited is None:
            visited = set()
    ...
    def _build_components(self):
        ...
        self._component_factory = SchematicComponentFactoryV2(self._tu_infos, self._interactions, self.connection_style)
        self._tu_components = {t: c for t in self._grid_coords if (c := self._component_factory.create_tu(t))}
        self._connection_components = [c for i, inter in enumerate(self._interactions) if (c := self._component_factory.create_connection(inter, i, self))]
        ...
        for sid, cells in source_cells.items():
            ...
            marker_tu = next((self._tu_infos[t] for t in tus if self._tu_infos.get(t, TUInfo(tu_id="", tu_name="")).is_marker), None)
            ...
            tag_parts = [p for p in [tag_tu.cotx_name or marker, tag_tu.aggregation_ratio_label] if p]

            anno = SourceAnnotation(
                id=f"anno_{sid}", parent=self, source_id=sid, has_tag=True,
    ...
    def _position_components(self, measured_sizes: Dict[str, Size]):
        rows, cols = self._grid_dimensions.get("num_rows", 0), self._grid_dimensions.get("num_cols", 0)
        ...
        self._grid_dimensions.update({
            "cell_widths": cell_w, "cell_heights": cell_h, "col_x": col_x, "row_y": row_y,
        ...
        for tu_id, (r, c) in self._grid_coords.items():
            if (comp := self._tu_components.get(tu_id)) and (sz := measured_sizes.get(tu_id)):
                comp.offset = Offset(absolute=(col_x[c] + (cell_w[c] - sz.width) / 2, row_y[r] + (cell_h[r] - sz.height) / 2))
                comp.parent = self
        ...
        for anno in self._annotation_components:
            ...
            cell_bounds = {(r, c): (col_x[c] + ml, row_y[r] + mt, col_x[c] + cell_w[c] - mr, row_y[r] + cell_h[r] - mb)
    ...
    def measure_and_layout(self, renderer: Optional[BaseRenderer] = None) -> Size:
        ...
        for c in self.children:
            if c and c.show and hasattr(c, "_dimensions") and isinstance(getattr(c, "offset", None), Offset) and c._dimensions.width >= 0:
                ox, oy = c.offset.absolute
                min_x, min_y = min(min_x, ox), min(min_y, oy)
                max_x, max_y = max(max_x, ox + c._dimensions.width), max(max_y, oy + c._dimensions.height)
