...
from jeanplot._deprecated.network_diagram import NetworkDiagram
...
from jeanplot.core.models import LayoutConstraints, BoxStyle, Size, Shadow
from jeanplot.core.renderer.matplotlib import MatplotlibRenderer
from jeanplot._deprecated.network_cotx_utils import inject_cotx_names_into_compute_graph
...
def render_circuit_schematic(mnet: Any, lib: Any, ax: Axes, **kw):
    # Handle both old ManagedNetwork (has build()) and new Network (has compute_graph)
    from biocomp.graphengine import GraphState

    if hasattr(mnet, "compute_graph") and isinstance(mnet.compute_graph, GraphState):
        # New-style Network with GraphState - use v2 components
        net = mnet
        schema = NetworkGeneticSchematicV2(network=net, **kw)
    elif hasattr(mnet, "build"):
        # Old-style ManagedNetwork - needs building, use v1 components
        mnet.build(lib)
        net = mnet.network
        inject_cotx_names_into_compute_graph(net)
        schema = NetworkGeneticSchematic(network=net, **kw)
    else:
        raise TypeError(f"Expected Network or ManagedNetwork, got {type(mnet)}")
