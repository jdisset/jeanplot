...
def _resolve_global_contours(X, Y, xlims, ylims, zslices, smooth_2d_params):
    ...
    for zentry in zslices:
        for pos in np.atleast_1d(zentry):
            _, ov = smooth_grid(X, Y, xlims, ylims, **{**knn_params, "zslice": np.atleast_1d(pos)})
            ov = np.asarray(ov)
...
def smooth_3d(
    ...
    contour_reference: tuple[NdArray, NdArray] | None = None,
    ...
    **_,
    ...
    if not np.all(np.isfinite(X)) or not np.all(np.isfinite(Y)):
        ...
        n_invalid = int(np.sum(~np.all(np.isfinite(X), axis=1) | ~np.all(np.isfinite(Y), axis=1)))
        logging.getLogger(__name__).warning(
            f"smooth_3d: filtering {n_invalid}/{len(X)} non-finite rows"
        )

    # Resolve symbolic contour levels (e.g. "otsu:0.65") from a reference field
    # when given, so a prediction cube can clip at the ground-truth iso-level.
    ref_X, ref_Y = contour_reference if contour_reference is not None else (X, Y)
    smooth_2d_params = _resolve_global_contours(
        ref_X, ref_Y, xlims, ylims, zslices, smooth_2d_params
    )
    ...
    def _slice_drawer(zslice, cbar_ax, slice_index):
        def _draw(sl_ax: Axes):
            ...
            for label in sl_ax.get_xticklabels() + sl_ax.get_yticklabels():
                label.set_bbox(
                    dict(facecolor="white", edgecolor="None", alpha=1, pad=0.75, zorder=1.5)
                )
            if cbar_ax is not None and im is not None:
                colorbar(
                    sl_ax, im, rescaler, vlims, label=output_name, cax=cbar_ax, **colorbar_params
                )
        ...
        for j, pos in enumerate(positions):
            ...
            if show_slice_ticks:
                ...
                slice_labels.append(
                    (
                        float(pos),
                        f"$ \\approx $ {format_powers(rescaler.inv(pos), n_decimals=0)}",
                        "slice",
                    )
                )
