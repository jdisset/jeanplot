...
from jeanplot.debug import debug_print  # Corrected import if needed, or use logging
...
def _generate_basic_orthogonal_path(
    start_np: np.ndarray,
    end_np: np.ndarray,
    start_dir_out: np.ndarray,  # expects OUTWARD direction for start
    end_dir_out: np.ndarray,  # expects OUTWARD direction for end
    start_len: float,
    end_len: float,
...
def create_orthogonal_path(
    ...
    start_dir_out: Tuple[float, float],  # Expects OUTWARD start direction
    end_dir_out: Tuple[float, float],  # Expects OUTWARD end direction
    ...
    if checkpoints:
        ...
        for i in range(len(nodes) - 1):
            ...
            if is_last_segment:
                ...
                current_end_dir = (
                    normalize_vector(tuple(-vec_to_next), default=(0, 0))
            ...
            if not is_last_segment and len(segment_points_tuples) >= 2:
                ...
                if np.linalg.norm(last_vec) > EPSILON:
                    current_start_dir = normalize_vector(tuple(last_vec), default=(1, 0))
