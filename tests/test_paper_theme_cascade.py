...
def test_paper_theme_fills_smooth2d_defaults():
    ...
    assert panel.style.padding.right == 0.6
...
def test_paper_theme_smooth1d_padding_right_default():
    jstyle.update(_load_paper_rules())
    panel = SmoothPanel1D()
    jstyle.apply(panel)
    assert panel.style.padding.right == 1.2


def test_paper_theme_smooth1d_padding_right_user_wins():
    jstyle.update(_load_paper_rules())
    panel = SmoothPanel1D(style={"padding": {"right": 0.4}})
    jstyle.apply(panel)
    assert panel.style.padding.right == 0.4
