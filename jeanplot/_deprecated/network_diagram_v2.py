    ...
    def _make_node(self, node, nid: int) -> ComputeNode | None:
        ...
        if cls is AggregationNode:
            ...
            for e in self._graph.get_incoming_edges(nid):
                src = self._graph.nodes.get(e.source_id)
                if src and src.node_type == "bias":
        ...
        if cls is TUNode:
            ...
            style, marker, cotx_name = ["source"], None, None
            if tu_id in tu_infos:
                info = tu_infos[tu_id]
                marker = info.cotx_marker
                cotx_name = info.cotx_name
                if marker:
                    style.extend([marker, "tu_marker"])
            tu = TUNode(style_class=style, **kw)  # no node_label, we add custom text
            if cotx_name and marker:
                ...
                tu.add_child(Text(text=cotx_name, style_class=["tu_label_prefix"], is_overlay=True))
            elif marker:
                tu.add_child(
