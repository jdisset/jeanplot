from typing import Optional, Union, Literal, List, Any, Dict, Sequence, Tuple
...
# re-importing ListOrSingle for clarity, although it's simple
...
class CellStyle(BoxStyle):
    # can disable/enable individual border sides (None = inherit default)
    ...
    # cell-specific dimension constraints (rarely needed, use column style)
    min_width: Optional[float] = None
    max_width: Optional[float] = None
    min_height: Optional[float] = None
    max_height: Optional[float] = None

    # content alignment within cell
    align_items: Optional[Literal["start", "center", "end", "stretch"]] = None
    justify_content: Optional[
        Literal["start", "center", "end", "space-between", "space-around", "space-evenly"]
    ] = None


class ColumnStyle(BaseModel):
    """Defines default style and width for a column"""

    width: Union[float, Literal["auto"], str] = (
        "auto"  # numeric=fixed, 'auto'=natural, 'X%'=percentage
    )
    # cell_style provides base styling for cells in this column
    cell_style: Optional[CellStyle] = Field(default_factory=CellStyle)
    ...
    def measure_and_layout(self, renderer=None) -> Size:
        ...
        if target_width >= 0 and self.min_dimensions.width == self.max_dimensions.width:
            ...
            if abs(self._dimensions.width - target_width) > 1e-6:
                # self._log_debug(f"Forcing width from {self._dimensions.width} to {target_width}") # Optional debug
...
class Table(Container):
    ...
    # controls how borders are drawn between cells
    border_collapse: Literal["collapse", "separate"] = "collapse"
    border_spacing: float = 0  # used only if border_collapse is 'separate'
    ...
    def measure_and_layout(self, renderer=None) -> Size:
        ...
        natural_size = super().measure_and_layout(renderer)

        # store natural widths
        ...
        # self._log_debug("Table final size", final_size)
        return final_size
    ...
    def _calculate_column_widths(
        ...
        # self._log_debug("Calculated Column Widths", widths)
        return widths

    def _log_debug(self, message: str, data=None):
        debug_print(self.id or "Table", message, data)
