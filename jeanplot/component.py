...
class Component(BaseModel):
    ...
    anchor_points: List["AnchorComponent"] = Field(default_factory=list)  # Use string forward ref
    ...
    def compute_local_matrix(self) -> np.ndarray:
        ...
        # if logger.isEnabledFor(logging.DEBUG): ... # Optional debug
        return matrix
    ...
    def compute_world_matrix(self, parent_world_matrix: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        comp_id_str = self.id or self.__class__.__name__
        # self._log_debug(f"compute_world_matrix: start ({comp_id_str})") # Optional debug
        self._resolve_attachment()
        intrinsic_transform_matrix = self.transform.to_matrix(self._dimensions)
        position_matrix: np.ndarray

        if self._resolved_attach_target:
            # self._log_debug("compute_world_matrix: using ATTACHED path") # Optional debug
            ...
            # self._log_debug("compute_world_matrix: using PARENT path") # Optional debug
            parent_world = (
                parent_world_matrix
            ...
            # self._log_debug("compute_world_matrix: using ROOT path") # Optional debug
        ...
        # self._log_debug(f"compute_world_matrix: final ({comp_id_str})", world_matrix) # Optional debug
        return world_matrix
    ...
    # Use string forward reference for BaseRenderer
    def _measure_natural(self, renderer: Optional["jeanplot.renderer.BaseRenderer"]) -> Size:
    ...
    def _apply_constraints(self, natural_size: Size) -> Size:
        ...
        # if final_size != natural_size: self._log_debug(...) # Optional debug
        return final_size

    # Use string forward reference for BaseRenderer
    def _layout_children(self, renderer: Optional["jeanplot.renderer.BaseRenderer"]):
        """positions child components. container subclasses override."""
        pass  # base component does nothing

    # Use string forward reference for BaseRenderer
    def measure_and_layout(self, renderer: Optional["jeanplot.renderer.BaseRenderer"] = None):
        """coordinates the measurement and layout process."""
        comp_id_str = self.id or self.__class__.__name__
        # self._log_debug(f"measure_and_layout: start ({comp_id_str})") # Optional debug
        self._apply_style()
        self._resolve_attachment()
        ...
        # self._log_debug(f"measure_and_layout: finished ({comp_id_str})") # Optional debug

    # Use string forward reference for BaseRenderer
    def render(self, renderer: "jeanplot.renderer.BaseRenderer", context: Any, matrix: np.ndarray):
    ...
    # Use string forward reference for Component
    def add_child(self, child: "Component"):
        ...
        if isinstance(child, AnchorComponent):
            ...
            # ensure parent link even if already in list
            child.parent = self
